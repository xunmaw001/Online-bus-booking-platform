<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback"
		:bottom="100">
		<view class="container">
			...
			<view class="detail-content">
				...
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.jifneduihuanNewMoney}}积分
					</view>
				</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.jifneduihuanContent"></rich-text>
				</view>
			</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
					<button @tap="onSubmitTap" style="margin-right: 10upx;min-width: 180upx;"
						...
						:style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[2],borderColor:btnColor[2]}">兑换</button>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			// 获取详情
			async init() {
				let res = await this.$api.info('jifneduihuan', this.id);
				this.detail = res.data;
				// 轮播图片
				this.swiperList = this.detail.jifneduihuanPhoto ? this.detail.jifneduihuanPhoto.split(",") : [];
			},
			...
			onSubmitTap(){
				...
				_this.$api.requestCondition('jifneduihuan',"duihuanwupin", data);
				_this.$utils.jump('/pages/jifenjilu/list?userid='+_this.user.id)
			},
		}
	}
</script>
